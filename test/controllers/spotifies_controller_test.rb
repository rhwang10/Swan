...
class SpotifiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create spotify" do
    assert_difference('Spotify.count') do
      post spotifies_url, params: { spotify: { status: @spotify.status, title: @spotify.title } }
    end
  ...
  test "should update spotify" do
    patch spotify_url(@spotify), params: { spotify: { status: @spotify.status, title: @spotify.title } }
